...
describe('AuthService', () => {
  ...
  describe('validateUser', () => {
    it('should return null if user not found', async () => {
      expect(await service.validateUser('nonexisting.user@example.com', 'secret')).toBe(null)
    })
    it('should return user object when valid email and password', async () => {
      expect(await service.validateUser('existing.user@example.com', 'secret')).toBe(existingUser)
    })

    it('should return null when invalid password', async () => { 
      expect(await service.validateUser('existing.user@example.com', 'wrong-secret')).toBe(null)
     })
  })
});
